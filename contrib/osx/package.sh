...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-dsv-macos
	cd /tmp/electrum-dsv-macos
...
rm -rf /tmp/electrum-dsv-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-dsv-macos/image/
cp -r $1 /tmp/electrum-dsv-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-DSV_uncompressed.dmg \
    /tmp/electrum-dsv-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-DSV_uncompressed.dmg electrum-dsv-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-DSV_uncompressed.dmg

echo "Done."
sha256sum electrum-dsv-$VERSION.dmg
