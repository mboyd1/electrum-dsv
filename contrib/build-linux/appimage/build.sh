...
cp -f "$PROJECT_ROOT/electrum_dsv/libsecp256k1.so.0" "$APPDIR/usr/lib/libsecp256k1.so.0" || fail "Could not copy libsecp to its destination"
...
(
    ...
    pushd "$CONTRIB"/deterministic-build/electrum-dsv-locale
    if ! which msgfmt > /dev/null 2>&1; then
        fail "Please install gettext"
    fi
    for i in ./locale/*; do
        dir="$PROJECT_ROOT/electrum_dsv/$i/LC_MESSAGES"
        mkdir -p $dir
...
cp "$PROJECT_ROOT/electrum_dsv/gui/icons/electrum-dsv.png" "$APPDIR/electrum-dsv.png"
