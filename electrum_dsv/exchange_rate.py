...
class Bit2C(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bit2c.co.il', '/Exchanges/DSVNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("DSV", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['DSV'][r]) for r in json['DSV']
                 if json['DSV'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=DSV")
        return json[ccy +'_DSV']


class Bitfinex(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitfinex.com', '/v1/pubticker/dsvusd')
...
class Bitso(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.bitso.com', '/v3/ticker/?book=dsv_mxn')
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/dsv{ccy.lower()}/')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=DSV')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/doriancoin/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/doriancoin/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/doriancoin?localization=false&tickers=false&community_data=false&developer_data=false&sparkline=false')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/doriancoin/market_chart?vs_currency=%s&days=max' % ccy)
...
class CoinSpot(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['dsv']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        dsv_prices = json['prices']['DSV']
        return dict([(r, Decimal(dsv_prices[r])) for r in dsv_prices])
...
class HitBTC(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ccys = ['USD']
        json = await self.get_json('api.hitbtc.com', '/api/2/public/ticker/DSV%s' % ccy)
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XDSVZ')]
        json = await self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XDSVZ'+ccy]['c'][0])
        return result
    ...
    async def request_history(self, ccy):
        query = '/0/public/OHLC?pair=DSV%s&interval=1440' % ccy
        json = await self.get_json('api.kraken.com', query)
        history = json['result']['XDSVZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=dsv_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('www.mercadobitcoin.net', '/api/dsv/ticker/')
...
class QuadrigaCX(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.quadrigacx.com', '/v2/ticker?book=dsv_cad')
...
class Bitragem(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.bitragem.com', '/v1/index?asset=DSV&market=BRL')
